...
func TestFindVMTemplateByTags_Return(t *testing.T) {
	m := NewMockClient(t)
	m.EXPECT().FindVMTemplateByTags(mock.Anything, []string{"tag"}, "subset", mock.Anything).
...
func TestFindVMTemplateByTags_RunAndReturn(t *testing.T) {
	m := NewMockClient(t)
	m.EXPECT().FindVMTemplateByTags(mock.Anything, []string{"a", "b"}, "exact", mock.Anything).
		RunAndReturn(func(_ context.Context, tags []string, policy string, _ string) (string, int32, error) {
