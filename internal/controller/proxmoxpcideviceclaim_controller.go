...
func (r *ProxmoxPCIDeviceClaimReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if claim.Status.BoundMappingID != "" {
		...
		markPCIClaimTrue(&claim, conditionReady)
	...
	if r.MappingProvider == nil {
		...
		markPCIClaimFalse(&claim, conditionReady, "NoProvider", clusterv1.ConditionSeverityWarning, "no mapping provider configured")
	...
	if err != nil {
		claim.Status.Phase = infrav1.ProxmoxPCIDeviceClaimPhaseFailed
		markPCIClaimFalse(&claim, conditionReady, "ListFailed", clusterv1.ConditionSeverityWarning, "%s", err.Error())
		_ = r.Status().Update(ctx, &claim)
	...
	if len(candidates) == 0 {
		...
		markPCIClaimFalse(&claim, conditionReady, "NoMatchingMapping", clusterv1.ConditionSeverityInfo, "no mapping matched selector")
		_ = r.Status().Update(ctx, &claim)
		return ctrl.Result{RequeueAfter: 30 * time.Second}, nil
	}
	...
	if chosen == nil {
		for i := range candidates {
			...
			if leaseErr != nil {
				claim.Status.Phase = infrav1.ProxmoxPCIDeviceClaimPhaseFailed
				markPCIClaimFalse(&claim, conditionReady, "LeaseError", clusterv1.ConditionSeverityWarning, "%s", leaseErr.Error())
				_ = r.Status().Update(ctx, &claim)
	...
	if chosen == nil {
		claim.Status.Phase = infrav1.ProxmoxPCIDeviceClaimPhasePending
		markPCIClaimFalse(&claim, conditionReady, "NoAvailableMapping", clusterv1.ConditionSeverityInfo, "no free mapping matched selector")
		_ = r.Status().Update(ctx, &claim)
...
func (r *ProxmoxPCIDeviceClaimReconciler) reconcileDelete(ctx context.Context, claim *infrav1.ProxmoxPCIDeviceClaim) (ctrl.Result, error) {
	...
	markPCIClaimFalse(claim, conditionReady, "Deleting", clusterv1.ConditionSeverityInfo, "claim is being deleted")
	_ = r.Status().Update(ctx, claim)
...
func markPCIClaimTrue(claim *infrav1.ProxmoxPCIDeviceClaim, conditionType string) {
	setPCIClaimCondition(claim, clusterv1.Condition{
...
func markPCIClaimFalse(claim *infrav1.ProxmoxPCIDeviceClaim, conditionType, reason string, severity clusterv1.ConditionSeverity, messageFormat string, args ...interface{}) {
...
func setPCIClaimCondition(claim *infrav1.ProxmoxPCIDeviceClaim, condition clusterv1.Condition) {
	conditions := claim.Status.Conditions
	for i := range conditions {
		if conditions[i].Type == condition.Type {
			if conditions[i].Status == condition.Status &&
				conditions[i].Severity == condition.Severity &&
	...
	claim.Status.Conditions = append(conditions, condition)
}
